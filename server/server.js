...
const db = mysql.createPool({
  ...
  connectionLimit: 1000, // Adjust as per your server capacity
...
app.post('/api/product', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    const sql = "INSERT INTO Product (productName, price, stock, Type, image, accountNumber) VALUES (?, ?, ?, ?, ?, ?)";
    const values = [req.body.name, req.body.price, req.body.stock, req.body.category, req.file.filename, req.body.accountNumber];
...
app.post('/buy', async (req, res) => {
  ...
  try {
    connection = await db.promise().getConnection();
    const orderSql = "INSERT INTO Orders (userID, status, totalAmount, shippingMethodID, accountNumber) VALUES (?, ?, ?, ?, ?)";
    const orderValues = [
      2,
